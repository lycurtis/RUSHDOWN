...
sensor.
To use it, you need to pass in the address to the DDR and PIN registers you are
using and the the pin number being used.
EX: IRinit(&PORTD, &PIND, 7); for PD7
To get a value from the remote, you need to create a decode_results variable. After
that, you can use
IRdecode() and pass in the variable's address. The function will return a 0 if it
fails to decode a value or
a 1 if it is successfully decode.
EX: IRdecode(&results)
After decoding, you will have the decoded value in results.value. To be able to get
a new value after a
...
int MATCH_MARK(int measured_ticks, int desired_us) {return MATCH(measured_ticks,
(desired_us + 100));}
int MATCH_SPACE(int measured_ticks, int desired_us) {return MATCH(measured_ticks,
(desired_us - 100));}
typedef struct {
